 ...
 */
@Path("/taverns")
public class TavernResource extends ResourceCollection<Tavern> {

    public TavernResource() {
        super(Tavern.class);
    }
    ...
    @Path("{id}")
    @Transactional
    public Tavern update(@PathParam("id") int id, Tavern src) {
        Tavern r = get(id);
        r.setWebsite(src.getWebsite());
        r.setName(src.getName());
        r.setPlacesId(src.getPlacesId());
        r.setEvent(src.getEvent());
        return manager.find(Tavern.class,id);
    }

    @POST
    @Consumes(APPLICATION_JSON)
    @Transactional
    public Response create(Tavern r) {
        manager.persist(r);
